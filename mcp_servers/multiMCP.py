...
class MultiMCP:
    ...
    async def function_wrapper(self, tool_name: str, *args):
        ...
        # Special handling for done tool to fix parameter schema
        if tool_name == "done" and "input" in params:
            # Convert message to text if needed
            if "message" in params["input"] and "text" not in params["input"]:
                message_value = params["input"].pop("message")
                if message_value is not None:
                    params["input"]["text"] = message_value
                else:
                    params["input"]["text"] = "Task completed successfully"
            elif "text" not in params["input"]:
                params["input"]["text"] = "Task completed successfully"
            
            # Ensure success field exists
            if "success" not in params["input"]:
                params["input"]["success"] = True
            
            # Re-call the tool with corrected parameters
            result = await self.call_tool(tool_name, params)

...
async def ensure_browser_session():
    ...
    if browser_session is None:
        ...
        try:
            # Try to get the current page to verify the session is still active
            page = await browser_session.get_current_page()
            if page.is_closed():
                # Page is closed, try to create a new one
                await browser_session.create_new_tab()
        except Exception as e:
            # Session is invalid, recreate it
            print(f"Browser session error: {e}. Recreating session...")
            try:
...
async def execute_controller_action(action_name: str, action_params=None, **kwargs) -> ActionResultOutput:
    ...
    try:
        ...
        # Get current page with error handling
        try:
            page = await browser_session.get_current_page()
        except Exception as e:
            if "Target page, context or browser has been closed" in str(e):
                # Session was closed, recreate it
                print("Browser context was closed. Recreating session...")
                await ensure_browser_session()
                page = await browser_session.get_current_page()
            else:
                raise e
        ...
        if action_name in ["open_tab", "go_to_url"]:
            try:
                ...
                if "Target page, context or browser has been closed" in str(e):
                    success = False
                    error_msg = f"Browser context was closed during navigation: {str(e)}"
                else:
                    raise e
            ...
            try:
                ...
                if "Target page, context or browser has been closed" in str(e):
                    return ActionResultOutput(
                        ...
                        error=f"Browser context was closed while getting page state: {str(e)}",
                        is_done=False
                    )
                else:
                    # For other errors, still return the original result
                    return ActionResultOutput(
                        success=success,
                        content=result_content,
                        error=error_msg,
        ...
        if "Target page, context or browser has been closed" in str(e):
            return ActionResultOutput(
                success=False, 
                error=f"Browser context was closed: {str(e)}"
            )

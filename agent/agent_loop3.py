...
class AgentLoop:
    def __init__(self, perception_prompt, decision_prompt, summarizer_prompt, multi_mcp, strategy="exploratory"):
        ...
        self.browser_agent = BrowserAgent(multi_mcp=multi_mcp)
    ...
    async def run(self, query: str):
        ...
        # Handle browser routing - DO NOT forward to Decision
        if self.p_out.get("route") == Route.BROWSER:
            log_step("🌐 Routing to BrowserAgent - bypassing Decision agent", symbol="🌐")
    ...
    async def _run_browser_agent(self):
        """Execute browser automation using BrowserAgent - completely separate from Decision agent."""
        log_step("🌐 Starting BrowserAgent execution - standalone mode", symbol="🌐")
        
        try:
            # Run browser agent with the original query
            browser_result = await self.browser_agent.run(self.query, session=self.session)
            
            # Update context with browser results
            self.ctx.globals.update({
                "browser_result": browser_result,
                "browser_status": browser_result.get("status"),
                "browser_steps_completed": browser_result.get("steps_completed", 0),
                "browser_final_result": browser_result.get("result")
            })
            
            # Mark browser step as completed
            browser_step_id = "BROWSER_AGENT"
            self.ctx.add_step(
                step_id=browser_step_id,
                description=f"Browser automation: {self.query}",
                step_type=StepType.CODE,
                from_node=StepType.ROOT
            )
            self.ctx.mark_step_completed(browser_step_id)
            
            # Create a perception snapshot for browser completion
            browser_perception = {
                "entities": self.p_out.get("entities", []),
                "result_requirement": self.p_out.get("result_requirement", ""),
                "original_goal_achieved": browser_result.get("status") == "completed",
                "reasoning": f"Browser automation completed with status: {browser_result.get('status')}",
                "local_goal_achieved": browser_result.get("status") == "completed",
                "local_reasoning": f"Browser agent executed {browser_result.get('steps_completed', 0)} steps",
                "last_tooluse_summary": f"BrowserAgent executed {browser_result.get('total_steps', 0)} steps",
                "solution_summary": str(browser_result.get("result", "Browser automation completed")),
                "confidence": "0.9" if browser_result.get("status") == "completed" else "0.5",
                "route": "summarize"
            }
            
            self.ctx.attach_perception(browser_step_id, browser_perception)
            
            # Summarize the browser results
            self.p_out = browser_perception
            self.status = "success"
    ...
    async def _run_decision_loop(self):
        """Executes initial decision and begins step execution - for NON-browser tasks only."""
    ...
    async def _execute_steps_loop(self):
        ...
        while tracker.should_continue():
            ...
            # Check for browser route in step results
            if self.p_out.get("route") == Route.BROWSER:
                log_step("🌐 Browser route detected in step result - switching to BrowserAgent", symbol="🌐")
                return await self._run_browser_agent()

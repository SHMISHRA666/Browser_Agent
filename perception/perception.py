...
class Perception:
    ...
    async def run(self, p_input: dict, session: Optional[AgentSession] = None) -> dict:
        ...
        try:
            # Updated required keys to include browser route
            ...
            # Handle browser route - DO NOT forward to Decision
            if output.get("route") == "browser":
                log_step("🌐 Browser automation route detected - routing to BrowserAgent", symbol="🌐")
                if "instruction_to_summarize" not in output:
                    output["instruction_to_summarize"] = "Not applicable for browser automation"
            
            # Handle summarize route
            elif output.get("route") == "summarize" and "instruction_to_summarize" not in output:
